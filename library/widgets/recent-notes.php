 ...
 */
 class Rolo_Widget_Recent_Notes extends WP_Widget {
	...
	function Rolo_Widget_Recent_Notes() {
		$widget_ops = array('classname' => 'widget_recent_notes', 'description' => __( 'The most recent notes' ) );
...
<?php
	...
	function widget( $args, $instance ) {
		...
		$notes = get_comments( array( 'number' => $number, 'status' => 'approve' ) );
		...
		if ( $notes ) {
			foreach ( (array) $notes as $note) {
				$output .=  '<li class="recentnote">' . /* translators: comments widget: 1: comment author, 2: post link */ sprintf(_x('%1$s : %2$s', 'widgets'), get_the_title($comment->comment_post_ID) ,  '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_comment_excerpt($comment->comment_post_ID) . '</a>') . '</li><!-- .recent-note -->';
			}
 		}
		$output .= '</ul>';
		$output .= $after_widget;
